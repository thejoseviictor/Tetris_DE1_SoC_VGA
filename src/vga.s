...
background_color:
    ...
    LDR R0, [R0]
    MOV R9, #9               @Carregando 9 (1001)
    LSL R9, #4               @Deslocando para o local correto no array com 32 bits
...
@Parâmetros:
@R0 = Coordenada de Referência X (Máximo 512)
@R1 = Coordenada de Referência Y (Máximo 512)
@R2 = Tamanho
@R3 = Cor
@R4 = Tipo: Quadrado ou Triângulo
draw_polygon:
    @Salvando os registradores na pilha:
    ...
    LSL R1, R1, #9          @Deslocando o "y" em 9 bits
    LSL R2, R2, #18         @Deslocando o "tamanho" em 18 bits
    LSL R3, R3, #22         @Deslocando o "cor" em 22 bits
    LSL R4, R4, #31         @Deslocando o "tipo" em 31 bits
    ADD R1, R1, R0          @Concatenando "x" e "y"
    ADD R1, R1, R2          @Concatenando "x", "y" e "tamanho"
    ADD R1, R1, R3          @Concatenando "x", "y", "tamanho" e "cor"
    ADD R1, R1, R4          @Concatenando "x", "y", "tamanho", "cor" e "tipo"
    @Carregando o endereço virtual:
    LDR R0, =virtual_address
    LDR R0, [R0]
    @Enviado os dados da instrução:
...
@R2 = Coordenada x (10 bits) (Máximo: 1023)
@R3 = Coordenada y (10 bits) (Máximo: 1023)
...
set_sprite:
    ...
    BL buffer_overflow
    @Montando a instrução:
    LSL R2, #19            @Deslocando o "x" em 9 bits
    LSL R3, #9             @Deslocando o "y" em 19 bits
    LSL R4, #29            @Deslocando o "sp" em 29 bits
    ADD R1, R1, R2         @Concatenando "offset" e "x"
    ADD R1, R1, R3         @Concatenando "offset", "x" e "y"
    ADD R1, R1, R4         @Concatenando "offset", "x", "y" e "sp"
    ...
    STR R1, [R0, #DATA_B]  @"offset", "x", "y", "sp"
    STR R4, [R0, #WRREG]   @Sinal de "start"
    STR R3, [R0, #WRREG]   @Desativando o sinal de "start"
    @Removendo os registradores da pilha e saindo da função:
    POP {R0, R1, R2, R3, R4, LR}
